...
for node in $(echo ${SLURM_NODELIST}|sed -e 's/,/ /g');do
    if [ ${node} != ${SLURMD_NODENAME} ];then
        ...
        mkdir -p /scratch/${SLURM_JOBID}/
        sleep 15
        logger --tag slurm_${SLURM_JOBID} "rsync --bwlimit=${BW_LIMIT} -aP /tmp/test.dd ${node}.${DOMAIN}:/scratch/${SLURM_JOBID}/"
        srun --exclusive -n1 rsync --bwlimit=${BW_LIMIT} -aP /tmp/test.dd ${node}.${DOMAIN}:/scratch/${SLURM_JOBID}/
        logger --tag slurm_${SLURM_JOBID} "Delete local file and file within /scratch/${SLURM_JOBID}/"
        srun --exclusive -n1 rm -f /scratch/${SLURM_JOBID}/test.dd /tmp/test.dd
        sleep 15
    fi
done
